...
function flip_screen()
	...
	if mobile then
		...
		love.graphics.rectangle(
			keys[0] and "fill" or "line",
			topad,
			screen_h - tobase * 3 - topad,
			tobase,
			tobase,
			topad,
			topad
		...
		love.graphics.rectangle(
			keys[2] and "fill" or "line",
			tobase + topad * 2,
			screen_h - tobase * 4 - topad * 2,
			tobase,
			tobase,
			topad,
			topad
...
function love.textinput(text)
	...
	if cart and pico8.cart._textinput then
